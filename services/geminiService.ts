...
// Fix: Always use standard imports and properties as per @google/genai guidelines
...
export const getCaddyCommentary = async (
  ...
  try {
    ...
    const response = await ai.models.generateContent({
      ...
      contents: prompt,
      config: {
        // Fix: Use thinkingBudget instead of thinkingLevel for minimal reasoning
        thinkingConfig: {
          thinkingBudget: 0,
        },
        temperature: 1.0, 
    ...
    // Fix: Access response text using the .text property as per guidelines
