...
export default function useAudio() {
  ...
  const stopAll = () => {
    bgSource.current?.pause();
    // Fix: Cast the array values to HTMLAudioElement[] to resolve 'Property does not exist on type unknown' error
    (Object.values(audioCache.current) as HTMLAudioElement[]).forEach(audio => {
      audio.pause();
      audio.currentTime = 0;
    });
    setBgPlaying(false);
